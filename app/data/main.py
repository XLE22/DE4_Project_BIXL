...
def create_poi_json() -> bool:
    try:
        ...
        if response2.status_code != 200:
            data_logger.error("Problème dans la création des données",exc_info=True)
            return False
        return True
    except requests.exceptions.RequestException:
        data_logger.error("Problème dans la création des données",exc_info=True)
...
def get_poi_file_path() -> str|bool:
    data_logger.info("Recherche du chemin pour atteindre le fichier POI.json")
    ...
    if os.path.exists(DATATOURISME_FOLDER):
        ...
        match len(files):
            case 0:
                data_logger.error("Le fichier POI n'existe pas",exc_info=True)
                return False
            case 1:
                return DATATOURISME_FOLDER + '/' + files[0]
            case _:
                data_logger.error("Il existe plus d'un fichier POI",exc_info=True)
                return False

    data_logger.error("Le répertoire POI n'existe pas",exc_info=True)
    return False


@app.get("/poi_json_file")
def get_poi_json_file(file_path: str):# -> str|bool:
    data_logger.info("Obtention du fichier POI.json")
...
@app.get("/")
def main() -> bool:
    data_logger.info("Lancement du micro service DATA")
